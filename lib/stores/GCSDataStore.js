...
class GCSDataStore extends DataStore {
    ...
    create(req) {
        return new Promise((resolve, reject) => {
            ...
            .on('error', reject)
            .on('finish', () => {
                this.emit(EVENTS.EVENT_FILE_CREATED, { file: file, upload_metadata: upload_metadata, req: req });
                resolve(file);
            });
        });
    }
            ...
            return new Promise((resolve, reject) => {
                ...
                req.on('end', () => {
                    ...
                    if (data.upload_length === new_offset) {
                        this.emit(EVENTS.EVENT_UPLOAD_COMPLETE, { file: file, metadata: data, req: req });
                    }
        ...
        return new Promise((resolve, reject) => {
            ...
            file.getMetadata((error, metadata, apiResponse) => {
                ...
                if (!('metadata' in metadata)) {
                    this.emit(EVENTS.EVENT_DUPLICATE_UPLOAD, { fileid: file_id , req:req });
                    return resolve(data);
                }
